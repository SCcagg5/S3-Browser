...
assert.match(style, /\.code-viewer\s*\{[^}]*overflow-x:\s*auto/s);
assert.match(style, /\.code-line-numbers\s*\{[^}]*position:\s*sticky[^}]*left:\s*0/s);
assert.match(style, /\.code-viewer \.code-column-guide\s*\{[^}]*repeating-linear-gradient[^}]*opacity:\s*\.55/s);
...
assert.match(apiSource, /async mediaProbe\(/);
assert.match(apiSource, /async createMediaSession\(/);
assert.match(apiSource, /async mediaSession\(/);
assert.match(apiSource, /async heartbeatMediaSession\(/);
assert.match(apiSource, /async deleteMediaSession\(/);
assert.match(apiSource, /\/api\/media-sessions/);

const actionsSource = read('src/public/assets/js/common/actions.js');
assert.match(actionsSource, /Folder details ready/);
...
assert.match(appSource, /showPermissionDetails\(permissionKey\)/);
...
assert.match(backendSource, /\/api\/media-source/);
assert.match(backendSource, /\/api\/media-probe/);
assert.match(backendSource, /\/api\/media-sessions/);
assert.match(backendSource, /\/api\/image-preview/);

const browserDockerfile = read('test/browser/Dockerfile');
assert.match(browserDockerfile, /apk add --no-cache ca-certificates ffmpeg imagemagick/);
...
assert.match(tabularSource, /controls\.append\(filterToggle, clear, pageSizeLabel\)/);
